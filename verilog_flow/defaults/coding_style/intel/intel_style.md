...
*   **MUST** use the implicit sensitivity list `always @(*)` or SystemVerilog's `always_comb` to describe combinational logic; explicitly listing all sensitivity signals is strictly prohibited.
...
*   **MUST** design clock enable (`ce`), set (`set`), and reset (`reset`) control signals as **Active-High**; active-low signals are strictly prohibited to avoid introducing additional inverter logic.
