...
- **4-space indentation**
- **Snake_case** for modules and signals
- **UPPER_SNAKE_CASE** for parameters

### Reset and Clock
- Read reset type from project config (`reset_type`, `reset_signal`)
- Default: async active-low (`rst_n`)
...
For each module in the spec:

1. **Create module header** with parameters and ports from spec
2. **Declare internal signals** for FSM states, pipeline registers, etc.
3. **Implement combinational logic** for next-state/output logic
4. **Implement sequential logic** for state registers and pipeline
